    ...
    toolsCtrl: function (e, data, source, oParent) {
        ...
        switch (el.attr('data-id')) {
            case 'addHeaderRows': // 新增表格行
                ...
                this.scope.$emit('Sortable:updateEvent', true);
                break;
            case 'addHeaderCols': // 新增表格列
                ...
                $timeout(function () {
                    ...
                    that.scope.$emit('Sortable:updateEvent', true);
                });
                break;
            ...
            case 'addDashboard': // 添加到聚合页
                if (!data.tables.headers.length || !this.scope.previewData[source.key]) {
