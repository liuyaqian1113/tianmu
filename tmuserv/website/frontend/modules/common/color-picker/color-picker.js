angular.module('ui.colorpicker', [])
    .directive('colorPicker', ['$compile', 'localize', '$timeout', function($compile, localize, $timeout) {
        return {
            restrict: 'EA',
            scope: {
                setColor: '&'
            ...
            link: function(scope, element, attrs) {
                var defaultColor = scope.setColor() || 'transparent';
                ...
                var template = '<div class="colorpicker-container" tabindex="-1" ng-click="keepPickerOpen($event)">'
                    ...
                    + ' ng-click="selectColorAndClose(transparent)"></div>'
                    + '</div>'
                ...
                var act = element[0].tagName === 'INPUT' ? 'input click' : 'click';
                element.on(act, openColorPicker);
                ...
                function openColorPicker(e) {
                    ...
                    var def = scope.setColor();
                    if (enable) {
                        e.stopPropagation();
                        var index = inArray(def, scope.latestColor || []);
                        if (index !== 0) {
                            $timeout(function () {
                                selectColor(def);
                            });
