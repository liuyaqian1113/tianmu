...
function (JTopo) {
    ...
    var getOffsetPosition = function (a) {
        return getOffsetPosition.__ret || (getOffsetPosition.__ret = (function () {
            if (!a)
                return {
                    left: 0,
                    top: 0
    ...
    function c(c, opt) {
        ...
        function i(a) {
            p && (window.clearTimeout(p),
                ...
                o = !1;
            var b = d(a);
            var _x = b.x * n.viewMode.pixel;
            var _y = b.y * n.viewMode.pixel;
        ...
        var q = "click,dbclick,mousedown,mouseup,mouseover,mouseout,mousemove,mousedrag,touchstart,touchmove,touchend,keydown,keyup".split(","),
            r = this;
        q.forEach(function (a) {
            ...
            this.setCenter = function (a, b) {
                ...
                this.childs.forEach(function (d) {
                    var e = a - c.canvas.width / 2 / c.viewMode.pixel,
                        f = b - c.canvas.height / 2 / c.viewMode.pixel;
                    d.translateX = -e,
                        d.translateY = -f
                })
            },
    ...
    function b(c) {
        ...
        this.initialize = function () {
            ...
            this.setBackground = function (a) {
                if (typeof a === 'string') {
                    ...
                    img.onload = img.complete = function () {
                        _this.background = this;
                        if (_this.stage.frames === 0) {
                            _this.paintBackgroud(_this.stage.graphics);
                        }
                        img.onload = img.complete = null;
                        img = null;
                    };
                    img.src = a;
                } else {
                    this.background = a;
                    if (this.stage.frames === 0) {
                        this.paintBackgroud(this.stage.graphics);
                    }  
                }
            },
            ...
            this.paintBackgroud = function (a) {
                var cache;
                if (!!this.background) {
                    var bg = typeof this.background === 'object' ? this.background.src : this.background;
                    cache = bg + this.alpha + a.canvas.width + a.canvas.height + '';
                    if (backgroundCache[this._id] === cache) {
                        return;
                    }
                   ...
                   // a.drawImage(this.background, 0, 0, a.canvas.width, a.canvas.height);
                    a.drawImage(this.background, 0, 0, this.background.width, this.background.height);
                } else if (!!this.backgroundColor) {
                    cache = this.backgroundColor + this.alpha + a.canvas.width + a.canvas.height + '';
                    if (backgroundCache[this._id] === cache) {
                        return;
                    }
                  //  a.clearRect(0, 0, a.canvas.width, a.canvas.height);
                ...
                backgroundCache[this._id] = cache;
                /*!!this.background ? backgroundCache[this._id] !== cache && a.drawImage(this.background, 0, 0, a.canvas.width, a.canvas.height) : null !== this.backgroundColor && backgroundCache[this._id] !== cache && (a.beginPath(),
                    a.fillStyle = "rgba(" + this.backgroundColor + "," + this.alpha + ")",
                    a.fillRect(0, 0, a.canvas.width, a.canvas.height),
                    a.closePath(), backgroundCache[this._id] = cache)*/
            },
                ...
                for (var c = 0; c < this.zIndexArray.length; c++)
                    for (var d = this.zIndexArray[c], e = this.zIndexMap[d], f = 0; f < e.length; f++) {
                        ...
                        if (g.id) {
                            ...
                            var flag = (g.id && nodeCache[g.id].x === g.x && nodeCache[g.id].y === g.y && nodeCache[g.id].width === g.width && nodeCache[g.id].height === g.height);
                            a.paintStatus = flag;
                        }
            ...
            this.toSceneEvent = function (b) {
                ...
                if (c.x /= this.scaleX,
                    ...
                    //            console.log(c.x, d);
                    c.x -= d.translateX / this.stage.viewMode.pixel,
                        c.y -= d.translateY / this.stage.viewMode.pixel
                }
            ...
            this.mousedragHandler = function (b) {
             ...
             //   console.log(c, b, this);
                this.mode == a.SceneMode.normal ? null == this.currentElement || this.currentElement instanceof a.Link ? 1 == this.translate && (this.stage.cursor = a.MouseCursor.closed_hand,
                    this.translateX = this.lastTranslateX + c.dx * this.stage.viewMode.pixel,
                    this.translateY = this.lastTranslateY + c.dy * this.stage.viewMode.pixel) : this.dragElements(c) : this.mode == a.SceneMode.drag ? 1 == this.translate && (this.stage.cursor = a.MouseCursor.closed_hand,
                    this.translateX = this.lastTranslateX + c.dx * this.stage.viewMode.pixel,
                    this.translateY = this.lastTranslateY + c.dy * this.stage.viewMode.pixel) : this.mode == a.SceneMode.select ? null != this.currentElement ? 1 == this.currentElement.dragable && this.dragElements(c) : 1 == this.areaSelect && this.areaSelectHandle(c) : this.mode == a.SceneMode.edit && (null == this.currentElement || this.currentElement instanceof a.Link ? 1 == this.translate && (this.stage.cursor = a.MouseCursor.closed_hand,
                    this.translateX = this.lastTranslateX + c.dx * this.stage.viewMode.pixel,
                    this.translateY = this.lastTranslateY + c.dy * this.stage.viewMode.pixel) : this.dragElements(c)); //,
...
function (a) {
    ...
    function c() {
        this.initialize = function () {
            ...
            this.isInBound = function (a, b) {
                var _x = this.x; // - _root.stage.viewMode.x;
                var _y = this.y; // - _root.stage.viewMode.y;
                var _r = this.x + this.width * Math.abs(this.scaleX);
                var _b = this.y + this.height * Math.abs(this.scaleY);
                //    console.log(a, b)
                //   console.log(a * _root.stage.viewMode.pixel,_x, _r,'|', b * _root.stage.viewMode.pixel, _y, _b, this,this.echartType, this.width, this.height);
                return a * _root.stage.viewMode.pixel > _x && a * _root.stage.viewMode.pixel < _r && b * _root.stage.viewMode.pixel > _y && b * _root.stage.viewMode.pixel < _b
            },
    ...
    function d() {
        ...
        this.paintCtrl = function (a) {
                var size = 20;
                if (!cornerCache[this.id]) {
                    cornerCache[this.id] = {};
                }
                var flag = (!this.selected && cornerCache[this.id].x === this.x && cornerCache[this.id].y === this.y && cornerCache[this.id].width === this.width && cornerCache[this.id].height === this.height)
                if (!!flag) {
                    return;
                }
                cornerCache[this.id] = {
                ...
                // console.log(this.selected, this.selectedPoint, this.x, this.y, this.width, this.height);
                if (0 != this.editAble) {
                    a.save();
                    for (var c = 0; c < b.length; c++) {
                        ...
                        d.left -= this.cx,
                            ...
                            d.bottom -= this.cy;
                        /*a.beginPath(),
                        a.fillStyle = "rgba(255,0,255,0.8)",
                        a.fillRect(d.left - 15, d.top - 15, 30, 30),
                        a.stroke(),
                        a.closePath()*/
                        var oCorner = this.oCorner;
                        if (!!oCorner) {
                            var w = oCorner.width;
                            var h = oCorner.height;
                            a.globalAlpha = !!this.selectedPoint ? 1 : .5;
                            a.drawImage(oCorner, d.left - w / 2 + 10, d.top - size, w, h);
                        } else {
                            a.beginPath(),
                                a.fillStyle = "rgba(255,0,255,0.8)",
                                a.fillRect(d.left - 15, d.top - 15, 30, 30),
            ...
            this.isInBound = function (a, c) {
                if (this.selectedPoint = null,
                    ...
                    for (var e = 0; e < b.length; e++) {
                        ...
                        //         console.log(a * _root.stage.viewMode.pixel, c * _root.stage.viewMode.pixel, f.left, f.right, f.top, f.bottom);
                        if (a * _root.stage.viewMode.pixel > f.left && a * _root.stage.viewMode.pixel < f.right + 15 && c * _root.stage.viewMode.pixel > f.top - 15 && c * _root.stage.viewMode.pixel < f.bottom + 15)
            ...
            this.mousedragHandler = function (a) {
                if (null == this.selectedPoint) {
                    ...
                    if ("Top_Left" == this.selectedPoint) {
                        ...
                        /* 等比缩放
                        var d = this.selectedSize.width + a.dx,
                            c = this.selectedLocation.y - a.dx;
                      
                        c < this.y + this.height && (this.y = c, this.height = this.selectedSize.height + a.dx), d > 1 && (this.width = d)
                        */
    ...
    function b(c) {
        this.initialize = function (c) {
                b.prototype.initialize.apply(this, arguments),
                    ...
                    this.text = c,
                    this.font = "24px Microsoft YaHei",
    ...
    function d(a) {
        this.initialize(),
            ...
            this.paint = function (a) {
                a.beginPath(),
                    a.font = this.font,
                    this.width = a.measureText(this.text)
                    .width,
                    this.height = a.measureText("田")
                    .width,
                    a.strokeStyle = "rgba(" + this.fontColor + ", " + this.alpha + ")",
                    a.fillStyle = "rgba(" + this.fontColor + ", " + this.alpha + ")",
        ...
        this.initialize = function (b, c, d) {
                if (f.prototype.initialize.apply(this, arguments),
                    ...
                    this.text = d,
                        ...
                        this.caculateIndex(),
                        this.font = "24px Consolas",
                        this.fontColor = this.strockColor || "255,255,255",
                        this.lineWidth = 2,
                        this.lineJoin = "miter",
                        this.transformAble = !1,
        ...
        this.paintText = function (a, b) {
                ...
                if (4 == b.length && (c = b[1],
                    ...
                    var g = a.measureText(this.text)
                        ...
                        h = a.measureText("田")
                        .width;
                    if (a.fillStyle = "rgba(" + (this.strokeColor || this.fontColor) + ", " + this.alpha + ")",
            ...
            this.isInBound = function (b, c) {
                ...
                for (var f = !1, g = 1; g < this.path.length; g++) {
                    ...
                    if (1 == a.util.isPointInLine({
                            x: b * _root.stage.viewMode.pixel,
                            y: c * _root.stage.viewMode.pixel
                        }, h, i)) {
                        f = !0;
                        break
                    }
