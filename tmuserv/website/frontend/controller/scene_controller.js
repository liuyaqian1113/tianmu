/**
 * @file scene_controller the file
 */
'use strict';
angular.module(window.ProjectName).controller('scene_controller',
    function ($rootScope, $scope, $state, $log, $stateParams, $timeout, $interval, CONFIG, fetchService) {
        ...
        $scope.canvasConf = {
            canvas: 'canvas-scene',
            consolePanelStatus: 0,
            mode: 'edit', //normal, edit
            bg: '' // CONFIG.webRoot + 'modules/common/scene/img/bg.jpg',
        };
        $scope.scene.setConsolePanelStatus = function () {
            $scope.$broadcast('Monitor:setConsolePanelStatus', {});
            console.log($scope.canvasConf);

        };
        $scope.scene.setBackgroundColor = function () {
            return $scope.$broadcast('Monitor:setBackgroundColor', {});
        };
        $scope.scene.setBackground = function () {
            return $scope.$broadcast('Monitor:setBackground', CONFIG.webRoot + 'modules/common/canvas/img/bg.jpg');
        };
        var ratio = '4:3';
        $scope.scene.setSceneRatio = function (a) {
            ratio = ratio === '4:3' ? '16:9' : '4:3';
            return $scope.$broadcast('Monitor:setSceneRatio', ratio);
        };
        ...
        $scope.scene.addNode = function () {
            // _this._createNode(v.x, v.y, v.text, ($ctrl.Image[v.level] || v.Image), v.textPosition, v.level, v.larm);
            return $scope.$broadcast('Monitor:addNode');
        };
        $scope.scene.addEcharts = function (type) {
            return $scope.$broadcast('Monitor:createEcharts', type);
        };
        $scope.scene.save = function () {
            return $scope.$broadcast('Monitor:save');
        };
        /*
        ...
        */
        ...
        (function () {
            ...
            fetchService.get([{
                ...
                url: api.getThemeConf,
                data: {
                    themeId: $state.params.themeId
                }
            }]).then(function (ret) {
                if (!!ret[0].type) {
                    return;
                }
                var category = ret[0].data;
                var config = ret[1].data;
                $scope.scene.themeCategory = category.data;
                $scope.scene.themeConf = config.data;
            });
        })();
